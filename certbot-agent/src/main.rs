/**
********************************************************************************
certbot-agent copies locally-registered Let's Encrypt certificates
to a remote server such as Minio. It's mostly useful for server machines
that can't run recent versions of certbot.
********************************************************************************
*/
...
fn scp_upload(source: String, dest: String, options: &[String]) -> Result<(), String> {
    let mut args: Vec<String> = options.to_owned();
    args.extend([source.into(), dest.into()]);

    run_command(SCP, &args)
}

// Run one or more commands on a remote host
fn ssh_exec(account: &str, cmd: String, options: &[String]) -> Result<(), String> {
    let mut args: Vec<String> = options.to_owned();
...
fn main() -> Result<(), Box<dyn Error>>  {
    ...
    if cert_timestamp > upload_timestamp.get() {
        ...
        upload_timestamp.commit()
            .expect("Error writing upload timestamp");
        Ok(())
    } else {
...
- The ssh remote command isn't escaped with "'" in error messages.
  The `shell-words' crate could be used to fix this.
*/
